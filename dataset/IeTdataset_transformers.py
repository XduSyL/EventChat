    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                #step1:先将<image>标识符移除
                sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()
                #step2:将<image>与Query之间加上一个'/n'
...
class EventChatDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        #tokenizer是传入进来的，由from_pretrain载入好的
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        #llava的数据集的json文件具有id，imgae，conversation三个字段
        if 'image' in sources[0]:
            image_file = self.list_data_dict[i]['image']
            image_folder = self.data_args.image_folder
            if self.data_args.event_folder:
                event_file = image_file.replace(".png", ".npy")
                event_folder = self.data_args.event_folder
            #此处的processor为CLIP的processor
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                ...
                #batch['events'] = torch.stack(events)
                batch['events'] = events
